...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    if (existing.length > 0) {
      const newQty = existing[0].quantity + parsedQuantity;
      if (newQty > 99) {
        return res.status(400).json({ message: 'Số lượng tối đa mỗi sản phẩm là 99.' });
      }
    ...
    const [result] = await pool.query(
      ...
      [req.user.id, productId, productName, priceFormatted || '', imageUrl || '', parsedQuantity],
    );
...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    await pool.query('UPDATE cart_items SET quantity = ? WHERE id = ?', [parsedQuantity, req.params.id]);
